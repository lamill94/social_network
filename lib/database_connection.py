...
# This class interacts with the DB - it wraps the underlying psycopg library that we're using
...
class DatabaseConnection:
    ...
    def connect(self):
        try:
            self.connection = psycopg.connect(f"postgresql://localhost/{self.DATABASE_NAME}", row_factory = dict_row)
    ...
    # This method executes a SQL query on the database (usually after it's been seeded)
